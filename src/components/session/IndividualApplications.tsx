...
import {formatMentoringDate, formatMentoringTime, getPaymentStatusStyle} from "@/lib/utils/session";
...
export function IndividualApplications({
   ...
   onReject
    ...
    const dateGroups = _.groupBy(application, app => {
        const mentoringById = mentoring.get(app.mentoringId);
        return mentoringById?.date || '';
    });

    return (
        <div className="space-y-6 text-gray-600">
            {Object.entries(dateGroups)
                .filter(([date]) => date) // 빈 날짜 제외
                .sort(([dateA], [dateB]) => new Date(dateA).getTime() - new Date(dateB).getTime())
                .map(([date, dateApplications]) => (
                <div key={date} className="border rounded-lg overflow-hidden">
                    ...
                    <div className="divide-y">
                        {dateApplications.map((application) => {
                            const mentoringById = mentoring.get(application.mentoringId);
                            if (!mentoringById) return null;
                                return (
                                            ...
                                            <div className="flex justify-between items-center mt-2">
                                                <div className="flex items-center gap-2 ml-11">
                                                    ...
                                                    <div className="border rounded px-1 py-0.5 text-sm text-gray-700">
                                                        {formatMentoringTime(mentoringById.time, session.duration)}
                                                    </div>
                                                </div>
